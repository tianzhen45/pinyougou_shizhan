    ...
    watch:{
        ids:{
            ...
            handler: function (newValue, oldValue) {
                if (this.ids.length == this.resultMap.collectionList.length && this.resultMap.collectionList.length>0) {
                    this.checkAll = true;
                } else {
                    this.checkAll = false;
                }
    ...
    methods: {
        ...
        queryByPageNo:function(pageNo){
            if (0 < pageNo && pageNo <= this.pages) {
                app.pageNum = pageNo;
                app.ids=[];
                app.findUserCollectionList();
            }
        },
        //用户秒杀订单
        findUserCollectionList:function () {
            axios.post("/collection/search.do?pageNum="+ this.pageNum + "&pageSize=" + this.pageSize+"&username="+this.username).then(function (response) {
                app.resultMap = response.data;
                app.collectionList = app.resultMap.list;
        ...
        //取消收藏
        uncollect:function (collectionId) {
            axios.get("/collection/delete.do?ids="+collectionId).then(function (response) {
        ...
        addToCart: function (itemId) {
            ...
            axios.get("http://cart.pinyougou.com/cart/addItemToCartList.do?itemId="
                + itemId + "&num=1", {"withCredentials": true})
